...
class MultiMCP:
    ...
    async def call_tool(self, tool_name: str, arguments: Any) -> Any:
        # Robust type handling for arguments
        if arguments is None:
            arguments = {}
        
        # Convert arguments to dict if not already
        if not isinstance(arguments, dict):
            try:
                arguments = dict(arguments) if hasattr(arguments, 'items') else {'input': str(arguments)}
            except Exception as e:
                print(f"❌ Failed to convert arguments to dict: {e}")
        ...
        print("in MultiMCP call_tool", entry)
        config = entry["config"]
        params = StdioServerParameters(
            command=sys.executable,
            args=[config["script"]],
